    ...
    row.innerHTML = `
      ...
      <td>
        <button onclick="generateCertificate('${reg._id}')"
          ...
          ${reg.certificateUrl ? '🔄 Re-gen' : '🏅 Generate'}
        </button>
      </td>
    `;
    tbody.appendChild(row);
...
async function generateCertificate(regId) {
  try {
    showPopup('⏳', 'Generating...', 'Please wait while the certificate is being created.', 'success');
    const r = await fetch(`/api/certificates/generate/${regId}`, { method: 'POST' });
    const d = await r.json();
    if (r.ok) {
      showPopup('🏅', 'Certificate Generated!', 'Certificate created and student notified.', 'success');
      await loadRegistrations();
      filterAttendance();
    } else {
      showPopup('❌', 'Failed', d.message, 'error');
    }
  } catch(e) {
    showPopup('❌', 'Error', 'Failed to generate certificate.', 'error');
  }
}
...
async function generateAllCertificates() {
  ...
  if (!eventId) { showPopup('⚠️', 'No Event Selected', 'Please select an event first.', 'error'); return; }
  try {
    showPopup('⏳', 'Generating...', 'Creating certificates for all attended students...', 'success');
    const r = await fetch(`/api/certificates/generate-all/${eventId}`, { method: 'POST' });
    const d = await r.json();
    if (r.ok) {
      showPopup('🏅', 'Done!', d.message, 'success');
      await loadRegistrations();
      filterAttendance();
    } else {
      showPopup('❌', 'Failed', d.message, 'error');
    ...
    showPopup('❌', 'Error', 'Failed to generate certificates.', 'error');
  }
}
    ...
    <table style="width: 100%; border-collapse: collapse;">
      <thead>
        <tr style="background: #f8f9fa; border-bottom: 2px solid #e0e0e0;">
          ...
          <th style="padding: 12px; text-align: center;">Student ID</th>
