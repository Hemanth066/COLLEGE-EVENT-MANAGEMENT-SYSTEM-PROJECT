...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Populate sidebar faculty card
  const name = currentFaculty.fullName || currentFaculty.name || currentFaculty.username || 'Faculty';
  const nameEl = document.getElementById('sidebarFacultyName');
...
function updateStats() {
  ...
  // Sync profile tab stats
  const pEvents = document.getElementById("profileStatEvents");
  const pRegs = document.getElementById("profileStatRegs");
  const pAvg = document.getElementById("profileStatAvg");
  if (pEvents) pEvents.textContent = allEvents.length;
  if (pRegs) pRegs.textContent = validRegistrations.length;
  if (pAvg) pAvg.textContent = avg;
}
...
function displayEvents() {
  ...
  allEvents.forEach(event => {
    ...
    eventCard.innerHTML = `
      <h4>ðŸ“Œ ${event.title}</h4>
      <p>${event.description || "No description provided"}</p>
      
      <div class="event-meta">
        <div class="meta-item">ðŸ“ ${event.venue || "TBA"}</div>
        <div class="meta-item">ðŸ“… ${event.date || "TBA"}</div>
        <div class="meta-item">ðŸ• ${event.time || "TBA"}</div>
      </div>
      
      <span class="badge success">ðŸ‘¥ ${registrations.length} Registered</span>
    ...
    row.innerHTML = `
      ...
      <td>
        <button class="attendance-btn ${reg.attended ? 'present' : 'absent'}" 
          ...
          ${reg.attended ? '✅ Present' : 'âŒ Absent'}
        </button>
      </td>
    `;
...
function displayAttendedStudents() {
  ...
  attendedStudents.forEach((reg, index) => {
    ...
    row.innerHTML = `
      ...
      <td>
        <button onclick="generateCertificate('${reg._id}')"
          style="padding:6px 12px;border-radius:8px;border:1px solid rgba(34,197,94,0.35);
                 background:rgba(34,197,94,0.12);color:#4ade80;font-size:12px;font-weight:600;
          ...
          title="${reg.certificateUrl ? 'Re-generate certificate' : 'Generate certificate'}">
          ${reg.certificateUrl ? 'ðŸ”„ Re-gen' : 'ðŸ… Generate'}
        </button>
      </td>
    `;
    tbody.appendChild(row);
...
// Auto-generate certificate for one student
async function generateCertificate(regId) {
  try {
    showPopup('â³', 'Generating...', 'Please wait while the certificate is being created.', 'success');
    const r = await fetch(`/api/certificates/generate/${regId}`, { method: 'POST' });
    const d = await r.json();
    if (r.ok) {
      showPopup('ðŸ…', 'Certificate Generated!', 'Certificate created and student notified.', 'success');
      await loadRegistrations();
      filterAttendance();
    } else {
      showPopup('âŒ', 'Failed', d.message, 'error');
    }
  } catch(e) {
    showPopup('âŒ', 'Error', 'Failed to generate certificate.', 'error');
  }
}

// Generate certificates for ALL attended students in selected event
async function generateAllCertificates() {
  const eventId = document.getElementById('filterEventAtt')?.value;
  if (!eventId) { showPopup('âš ï¸', 'No Event Selected', 'Please select an event first.', 'error'); return; }
  try {
    showPopup('â³', 'Generating...', 'Creating certificates for all attended students...', 'success');
    const r = await fetch(`/api/certificates/generate-all/${eventId}`, { method: 'POST' });
    const d = await r.json();
    if (r.ok) {
      showPopup('ðŸ…', 'Done!', d.message, 'success');
      await loadRegistrations();
      filterAttendance();
    } else {
      showPopup('âŒ', 'Failed', d.message, 'error');
    ...
    showPopup('âŒ', 'Error', 'Failed to generate certificates.', 'error');
  }
}

// Toggle attendance
// NOTE: Attendance toggle is called from the Registrations tab.
async function toggleAttendance(regId, attended) {
  ...
  try {
    ...
    if (!regId) {
      showPopup('âŒ', 'Error', 'Registration ID is missing', 'error');
      return;
    }
    ...
    if (!response.ok) {
      ...
      showPopup('âŒ', 'Server Error', `Error ${response.status}: ${text}`, 'error');
      return;
    }
    ...
    if (response.ok) {
      ...
      showPopup('âŒ', 'Error', result.message, 'error');
    }
  } catch (error) {
    console.error("Error updating attendance:", error);
    showPopup('âŒ', 'Server Error', error.message, 'error');
  }
}
...
function populateEventFilters() {
  ...
  regFilter.innerHTML = '<option value="">ðŸ” Select an event to view registrations...</option>';
  attFilter.innerHTML = '<option value="">ðŸ” Select an event to view attendance...</option>';
  updateSelect.innerHTML = '<option value="">ðŸ” Select an event to update...</option>';
  certFilter.innerHTML = '<option value="">ðŸ” Select an event to upload certificates...</option>';
...
async function changeFacultyPassword() {
  ...
  if (!currentPwd || !newPwd || !confirmPwd) {
    showPopup('âš ï¸', 'Missing Fields', 'Please fill in all password fields.', 'error');
    return;
  }
  if (newPwd.length < 6) {
    showPopup('âš ï¸', 'Too Short', 'New password must be at least 6 characters.', 'error');
    return;
  }
  if (newPwd !== confirmPwd) {
    showPopup('âš ï¸', 'Mismatch', 'New password and confirm password do not match.', 'error');
    return;
  }
...
async function saveAllScores() {
  ...
  if (!eventId) {
    showPopup('âš ï¸', 'No Event Selected', 'Please select an event first', 'error');
    return;
  }
  ...
  if (!attendedStudents.length) {
    showPopup('âš ï¸', 'No Students Found', 'No attended students to assign scores for this event', 'error');
    return;
  }
...
// Download PDF Report for selected event registrations
function downloadEventReport() {
  ...
  if (!event) { showPopup('❌', 'Error', 'Event not found.', 'error'); return; }

  const filtered = allRegistrations.filter(reg => {
    const rid = typeof reg.eventId === 'string' ? reg.eventId : reg.eventId?._id;
    return rid === eventId;
  });
  ...
  const html = `
    ...
    <meta charset="UTF-8">
    <title>Registration Report - ${event.title}</title>
    <style>
    ...
    <h1>📋 ${event.title} — Registration Report</h1>
...
async function handleUpdateEvent(e) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      showPopup('âŒ', 'Update Failed', result.message, 'error');
    }
  } catch (error) {
    showPopup('âŒ', 'Server Error', 'Failed to update event. Please try again.', 'error');
    console.error(error);
  }
}
...
async function handlePublishEvent(e) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      showPopup('âŒ', 'Publish Failed', result.message || 'Unknown error occurred', 'error');
    }
  } catch (error) {
    console.error('Error publishing event:', error);
    showPopup('âŒ', 'Server Error', 'Failed to connect to server. Make sure the server is running on port 5000.', 'error');
  }
}
...
async function loadProfile() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (avatarEl) avatarEl.textContent = (profile.fullName || 'F').charAt(0).toUpperCase();

      // Header name / username
      const nameEl = document.getElementById('profileFullName');
      if (nameEl) nameEl.textContent = profile.fullName || 'Faculty Name';
      const userEl = document.getElementById('profileUsername');
      if (userEl) userEl.textContent = '@' + (profile.username || '');

      // Sync sidebar card
      const sidebarName = document.getElementById('sidebarFacultyName');
      const sidebarAvatar = document.getElementById('sidebarAvatar');
      const displayName = profile.fullName || profile.username || 'Faculty';
      ...
      set('profileFullNameDisplay', profile.fullName);
      set('profileDepartmentDisplay', profile.department);
      set('profileEmailDisplay', profile.email);
      set('profilePhoneDisplay', profile.phone);

      // Edit form fields
      const setVal = (id, val) => { const el = document.getElementById(id); if (el) el.value = val || ''; };
      setVal('profileFullNameInput', profile.fullName);
      setVal('profileEmail', profile.email);
      setVal('profilePhone', profile.phone);
      setVal('profileDepartment', profile.department);
    } else {
      console.error('Failed to load profile:', response.status);
      showPopup('âš ï¸', 'Profile Not Found', 'Please update your profile information.', 'error');
    }
  } catch (error) {
    console.error('Error loading profile:', error);
    showPopup('âŒ', 'Error', 'Failed to load profile. Please try again.', 'error');
  }
}
...
async function updateProfile(e) {
  ...
  try {
    ...
    showPopup('âŒ', 'Server Error', 'Failed to update profile. Please try again.', 'error');
  }
}
...
function displayEventPerformance() {
  ...
  if (allEvents.length === 0) {
    container.innerHTML = '<p style="color:rgba(255,255,255,0.4); text-align:center; padding:40px;">No events published yet</p>';
    return;
  }
      ...
      <tbody>
        ${eventStats.map(stat => `
          <tr style="border-bottom:1px solid rgba(255,255,255,0.04);">
            <td style="padding:13px 14px; font-weight:500; color:rgba(255,255,255,0.85);">${stat.title}</td>
            <td style="padding:13px 14px; text-align:center;">
              <span class="badge" style="background:rgba(59,130,246,0.12); color:#60a5fa; padding: 4px 12px; border-radius: 12px; font-size: 13px;">
                ${stat.registrations}
              </span>
            </td>
            <td style="padding:13px 14px; text-align:center;">
              <span class="badge" style="background:rgba(34,197,94,0.12); color:#4ade80; padding: 4px 12px; border-radius: 12px; font-size: 13px;">
                ${stat.attended}
              </span>
            </td>
            <td style="padding:13px 14px; text-align:center;">
...
function displayTopStudents() {
  ...
  if (allRegistrations.length === 0) {
    container.innerHTML = '<p style="color:rgba(255,255,255,0.4); text-align:center; padding:40px;">No registrations yet</p>';
    return;
  }
  ...
  if (topStudents.length === 0) {
    container.innerHTML = '<p style="color:rgba(255,255,255,0.4); text-align:center; padding:40px;">No scores assigned yet</p>';
    return;
  }
  ...
  container.innerHTML = `
    <table style="width: 100%; border-collapse: collapse;">
      <thead>
        <tr style="background:rgba(255,255,255,0.05); border-bottom:1px solid rgba(255,255,255,0.1);">
          <th style="padding:12px 14px; text-align:left; color:rgba(255,255,255,0.5); font-size:11px; text-transform:uppercase; letter-spacing:0.6px;">Rank</th>
          <th style="padding:12px 14px; text-align:left; color:rgba(255,255,255,0.5); font-size:11px; text-transform:uppercase; letter-spacing:0.6px;">Student Name</th>
          <th style="padding:12px 14px; text-align:center; color:rgba(255,255,255,0.5); font-size:11px; text-transform:uppercase; letter-spacing:0.6px;">Student ID</th>
          <th style="padding:12px 14px; text-align:center; color:rgba(255,255,255,0.5); font-size:11px; text-transform:uppercase; letter-spacing:0.6px;">Events Attended</th>
          <th style="padding:12px 14px; text-align:center; color:rgba(255,255,255,0.5); font-size:11px; text-transform:uppercase; letter-spacing:0.6px;">Total Score</th>
        </tr>
      </thead>
      <tbody>
        ${topStudents.map((student, index) => `
          <tr style="border-bottom:1px solid rgba(255,255,255,0.04);">
            ...
            <td style="padding:13px 14px; font-weight:500; color:rgba(255,255,255,0.85);">${student.name}</td>
            <td style="padding:13px 14px; text-align:center; color:rgba(255,255,255,0.55);">${student.pin}</td>
            <td style="padding:13px 14px; text-align:center;">
              <span class="badge" style="background:rgba(59,130,246,0.12); color:#60a5fa; padding: 4px 12px; border-radius: 12px; font-size: 13px;">
            ...
            <td style="padding:13px 14px; text-align:center;">
              <span style="font-size:18px; font-weight:700; color:#818cf8;">
                ${student.totalScore}
              </span>
            </td>
          </tr>
...
function populateFeedbackEventDropdown() {
  ...
  select.innerHTML = '<option value="">ðŸ” Select an event to view feedback...</option>';
  allEvents.forEach(event => {
...
async function loadFeedbackAnalysis() {
  ...
  try {
    ...
    if (feedbacks.length === 0) {
      ...
      document.getElementById('fbAvgRating').textContent = 'â€”';
      document.getElementById('fbStars').textContent = 'â€”';
